...
public class SMVersion2ImplHandler  implements SM {
  
  private final static boolean DEBUG = true ;
  private final static int SMALL_REC_SIZE = Device.BLOCK_SIZE ;
  private Hashtable buffer = new Hashtable(); 
  private static Map<SM.OID,String> mappedValues = new HashMap<SM.OID,String>();
  private Volume master_volume = null ;  
  private Volume data_volume = null ;     
  private Volume index_volume = null ;    
  private ByteBuffer translation_table = null ; 
  private Hashtable lookupcache = new Hashtable() ;
  private static SMVersion2ImplHandler sm = null;
  static{
	  try {
		sm = initialize();
	} catch (Exception e) {
   ...
   *@since
   */
  public static void storeCompleteData(String args) {
    ...
    try {
      ...
      // create a new SM
    	SMVersion2ImplHandler sm = new SMVersion2ImplHandler();
  ...
  public Record fetch(SM.OID oid) throws NotFoundException, IOException {
    ...
    try {
      ...
      if (DEBUG & recid != null)
        System.out.println( "Fetch: " + Util.toHexString(recid) ) ;
  ...
  public static boolean updateRecord(Call tranzReqDTO){
	  
	  boolean isUpdated = false; 
	  
	  try {
	  Record rec1 = new Record(20);
	  rec1.setBytes(tranzReqDTO.getValue().getBytes());
	  
	  if(mappedValues.containsValue(tranzReqDTO.getKey())){
	   	  
		  
	    	  for (SM.OID key : mappedValues.keySet()) {
	    		  if(tranzReqDTO.getKey().equals(mappedValues.get(key))){
  ...
  public static boolean deleteR1(String mykey){
	  
	  boolean del = false;
	  SM.OID smkey = null;
	  if(mappedValues.containsValue(mykey)){
		  for (SM.OID key : mappedValues.keySet()) {
		    	
    		  if(mykey.equals(mappedValues.get(key))){
    			  smkey = key;
  ...
  public SM.OID getOID( byte[] oidbytes ) {
	return new SMVersion2ImplHandler.OID( oidbytes ) ;	  
  }
  public class OID implements SM.OID {
    ...
    public byte[] toBytes() {
	  ...
	  public static SMVersion2ImplHandler initialize() throws Exception{
		 SMVersion2ImplHandler sm = new SMVersion2ImplHandler();
	    ...
	    public static boolean storeD1(Call txReqDTO){
		...
		try {
			...
			rec1 = new Record(20);
			rec1.setBytes(txReqDTO.getValue().getBytes());
			System.out.println( "Storing: " ) ;
	        rec1_oid = (SM.OID) sm.store(rec1);
	        mappedValues.put(rec1_oid, txReqDTO.getKey());
	        dataStored = true;
	  ...
	  public static String getD2(String userName) {
	    ...
	    Record found = null;
	      ...
	      if(mappedValues.containsValue(userName)){  
	    	  
		      try {
		    	  
		    	  for (SM.OID key : mappedValues.keySet()) {
		    	
		    		  if(userName.equals(mappedValues.get(key))){
		    			 
		    			  found = sm.fetch(key);

		    			  String resStr = new String(found.getBytes(0,0));
		    			  resStr = resStr.replaceAll("\\s+$", ""); 
		    			  
		    			  jsonOb.put(userName,resStr.trim());
		    			  cleanVal= jsonOb.toString();
		    		  }
	    	  ...
	    	  System.out.println("OID Record not found.");
	    	  cleanVal = "";
	      }
